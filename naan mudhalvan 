...
def get_recommendations(user_id, all_items, model):
    item_ids = list(set(interaction_df['item_id'].unique()).union(set(range(101, 106))))
    predictions = [model.predict(user_id, iid) for iid in item_ids if iid not in interaction_df[interaction_df['user_id'] == user_id]['item_id'].values]
